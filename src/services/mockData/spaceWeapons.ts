...
export const MAIN_WEAPONS: Item[] = [
  ...
  },
  {
    id: 'ion_cannon',
    name: 'Ion Cannon',
    description: 'Heavy weapon that disrupts shields and armor with electromagnetic pulses.',
    type: 'weapon',
    rarity: 'uncommon',
    marketValue: 12000,
    tradeable: true,
  ...
  },
  {
    id: 'gauss_launcher',
    name: 'Gauss Launcher',
    description: 'Electromagnetic projectile weapon. Devastating at close range.',
    type: 'weapon',
    rarity: 'rare',
    marketValue: 25000,
    tradeable: true,
...
export const getItemsByType = (type: 'main' | 'secondary' | 'temp' | 'armor') => {
  switch (type) {
    case 'main':
      return MAIN_WEAPONS
    case 'secondary':
...
export const categorizeWeapon = (itemName: string): 'main' | 'secondary' | 'temp' | null => {
  const name = itemName.toLowerCase()
  
  if (name.includes('rifle') || name.includes('cannon') || name.includes('launcher') || name.includes('beam')) {
    return 'main'
  }
  if (name.includes('pistol') || name.includes('blaster') || name.includes('sidearm')) {
    return 'secondary'
  }
  if (name.includes('grenade') || name.includes('mine') || name.includes('bomb')) {
    return 'temp'
  }
